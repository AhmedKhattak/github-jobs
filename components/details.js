...
export function Details({ show = false, handleClose }) {
  ...
  return (
    <>
      {!show && toggleLayout && (
        <p className="mt-6 sticky top-2 self-start ">Select a job !</p>
      )}
      <AnimatePresence>
        {show && (
          <motion.div
            ...
            transition={{ duration: 0.24, easings: "easeInOut" }}
            className={`${
              toggleLayout
                ? "sticky top-2  w-7/12 hidden md-plus:block   text-white self-start"
                : "fixed  left-0  top-0 right-0 z-[999]  rounded-t-md  md-plus:block   text-white self-start"
            }`}
          >
            <div className="absolute  h-32    p-5 top-0 pt-6 border-l-2 border-r-2 border-t-2 border-b-2 border-black    bg-gray-900 left-0 right-0  z-50  items-center flex">
              <div
                onClick={handleClose}
            ...
            <div
              style={{ height: "100vh" }}
              className="overflow-scroll rounded-b-md border-l-2 border-r-2 border-b-2 border-gray-600  bg-white shadow-md prose max-w-none p-5 w-full"
            >
              <ReactMarkdown
